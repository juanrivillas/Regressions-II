...
Other option to check loaded data:
...
data1$wc[is.na(data1$wc)] <- mean(data1$wc, na.rm=TRUE)
data1$height[is.na(data1$height)] <- mean(data1$height, na.rm=TRUE)
data1$weight[is.na(data1$weight)] <- mean(data1$weight, na.rm=TRUE)

data3    <- na.omit(data2) #this dataset includes complete case (removed occupation n=347 and incomplete markers) 
...
```{r}
Freq(data1$height, useNA = "always")
...
```{r}
table_test <- table1(~ethnic_group | sex, data=biomarkers)
